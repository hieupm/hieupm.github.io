...
sampleDF.select($"id", callUDF("capitalizeFirstUsingSpace", $"text").as("text")).show
```

```java
val capitalizerUDF = udf ((fullString: String, splitter: String) => fullString.split(splitter).map(_capitalize).mkString(slippter))
